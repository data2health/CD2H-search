...
<ol class="bulletedList">
	<rdf:foreachConcept var="x" queryString="${rewrittenQuery}">
		<rdf:concept>
			...
			<li><a href="${param.target_page}?mode=concept&query=${fn:replace(fn:replace(param.query,'&','%26'), concept, cui)}${query_suffix}${param.drilling}"><rdf:conceptCUI /></a>	- <rdf:conceptPhrase />
			<ol class="bulletedList">
				<li>Super concepts:
					<ol class="bulletedList">
						<rdf:foreachSuperconcept var="">
							<c:set var="supercui"><rdf:superconceptCUI /></c:set>
							<li><a href="${param.target_page}?mode=concept&query=${fn:replace(fn:replace(fn:replace(param.query,'&','%26'), concept, supercui), cui, supercui)}${query_suffix}${param.drilling}"><rdf:superconceptCUI /></a> - <rdf:superconceptPhrase />
				...
				<li>Sub concepts:
					<ol class="bulletedList">
						<rdf:foreachSubconcept var="">
							...
							<li><a href="${param.target_page}?mode=concept&query=${fn:replace(fn:replace(fn:replace(param.query,'&','%26'), concept, subcui), cui, subcui)}${query_suffix}${param.drilling}"><rdf:subconceptCUI /></a> - <rdf:subconceptPhrase />
