...
<body class="home page-template-default page page-id-6 CD2H">
	...
	<div class="container-fluid" style="padding-left:5%; padding-right:5%;">
		<br/> <br/> 
		<div class="col">
			<h2><i style="color:#7bbac6;"class="fas fa-search"></i>CTSA Hub Services Search</h2>
			<div id=form>
				<form method='POST' action='serviceSearch.jsp'>
					<fieldset>
						...
						<c:if test="${not empty param.query}">
							<a class="search-reset" href="serviceSearch.jsp" title="Reset Search"><i class="far fa-times-circle"></i></a>
						</c:if>
					</fieldset>
				</form>
			</div>
			...
			<c:when test="${not empty param.query}">
				...
				<lucene:taxonomy taxonomyPath="/usr/local/CD2H/lucene/hub_services_tax">
                    ...
                    <lucene:countFacetRequest categoryPath="Service" />
					...
					<lucene:search lucenePath="/usr/local/CD2H/lucene/hub_services" label="content" queryParserName="boolean" queryString="${param.query}" useConjunctionByDefault="true">
						<div style="with: 100%">
																...
																<c:choose>
																	<c:when test="${count_children == 0 and not fn:contains(drillDownList, facet2path.concat('|')) and not fn:contains(drillDownList, facet2path)}">
																		<li>
																			...
																			<div class = 'facet-list-item'>
																				<a href="serviceSearch.jsp?query=${param.query}&drillDown=${drillDownList}${facet2path}">${facet2}</a> (<lucene:facet label="count" />)
																			</div>
																	</c:when>
																	<c:when	test="${fn:contains(drillDownList, facet2path.concat('|')) and count_children ==0}">
																		<li>
																			...
																			<div class = 'facet-list-item'>
																				<lucene:facet label="content" /> <a	class="facet-move" href="serviceSearch.jsp?query=${param.query}&drillDown=${drillDownList}&drillUp=${facet2path}" title="Remove Filter"><i class="far fa-times-circle"></i></a>
																			</div>
																	</c:when>
																			...
																			<lucene:facet label="none">
																				<c:choose>
																					<c:when	test="${fn:contains(drillDownList, facet3path.concat('|'))}">
																						...
																						<a	class='facet-move' href="serviceSearch.jsp?query=${param.query}&drillDown=${drillDownList}&drillOut=${facet3path}" title="Remove This Filter"><i class="far fa-arrow-alt-circle-left"></i></a>
				                                                                      	<a  class='facet-move' href="serviceSearch.jsp?query=${param.query}&drillDown=${drillDownList}&drillUp=${facet3path}" title="Remove All Filters in Category"><i class="far fa-times-circle"></i></a>
		        																	</c:when>
																					<c:when	test="${fn:contains(drillDownList, facet3path)}">
																						<li><lucene:facet label="content" />
																					</c:when>
																					<c:otherwise>
																						<li><a	href="serviceSearch.jsp?query=${param.query}&drillDown=${drillDownList}${facet3path}">${facet3}</a> (<lucene:facet label="count" />)
																					</c:otherwise>
																				</c:choose></li>
												                              </lucene:facet>
											                           </lucene:facetIterator>
            ...
            <p>This interface is a very preliminary implementation containing only hub services,<br>
            and is primarily intended to support tuning of the matching and ranking algorithms.<br>
            At this point in our work, coverage is fairly modest, but will be improving over time.<br>
            Once results reach a "reasonable" level of performance, this will fold into the main interface.<br>
            See <a href="facetSearch.jsp">CD2H Faceted Search</a> for the interface containing all content types.
            </p>
 	</div>
